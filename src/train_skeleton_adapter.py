...
class Net(nn.Module):
    ...
    def forward(
        self,
        inp_noisy_latents, timesteps, encoder_hidden_states, added_time_ids,
        pose_values,
    ):

        pose_latents = self.pose_guider(pose_values)
        ...
        # model_pred = self.unet(
        #     inp_noisy_latents, timesteps, encoder_hidden_states, added_time_ids=added_time_ids, pose_latents=pose_latents
        # ).sample
        return model_pred

import decord
class DummyDataset(Dataset):
    def __init__(self, num_samples=100000, width=576, height=1024, sample_frames=25):
        ...
        self.total_list = os.listdir('../tiktok/tiktok_videos_pose_videos_bodyhand_rescale1024_slim_edition/')

    def __len__(self):
        return len(self.total_list)

    def __getitem__(self, idx):
        video_path = '../tiktok/tiktok_videos_pose_videos_bodyhand_rescale1024_slim_edition/'+self.total_list[idx]
        skr_path = '../tiktok/tiktok_videos_pose_videos_onlybody_rescale1024_slim_edition/'+self.total_list[idx]
        vr = decord.VideoReader(video_path)
        skr = decord.VideoReader(skr_path)

        ################## modified by yuhang ##################
        clip_length = min(len(vr), (self.sample_frames - 1) * 4 + 1)
        ...
        for i, index in enumerate(batch_index):
            img = skr[index]
            output_pose.append(cv2.cvtColor(cv2.resize(np.array(Image.fromarray(img.asnumpy())), (self.width, self.height)), cv2.COLOR_BGR2RGB).transpose(2, 0, 1))
        output_pose = np.stack(output_pose)
        output_pose = torch.from_numpy(output_pose.copy()) / 127.5 - 1
        # pose_pixels: fx3x576x1024
        # pose_pixels = output_pose.unsqueeze(0).to(device)



        selected_frames = vr.get_batch(batch_index).asnumpy()
        pixel_values = torch.empty((self.sample_frames, self.channels, self.height, self.width))
        for i in range(selected_frames.shape[0]):
            img = selected_frames[i]
            img = np.clip(img, 0, 255).astype(np.uint8)
            img_resized = cv2.resize(img, (self.width, self.height))
            # img = np.clip(img, 0, 255) 
            # img_pil = Image.fromarray(np.uint8(img))
            # img_resized = img_pil.resize((self.width, self.height))
...
import sys
sys.path.append('/cpfs/data/user/yuhang/MimicMotion/mimicmotion/modules')
from unet import UNetSpatioTemporalConditionModel
...
def main():
    ...
    mimicmotion_models = MimicMotionModel("../MimicMotion/models/SVD/stable-video-diffusion-img2vid-xt-1-1").to(device=device).eval()
    # mimicmotion_models.load_state_dict(torch.load("../MimicMotion/models/MimicMotion.pth", map_location=device), strict=False)
    ...
    # checkpoint = torch.load("../MimicMotion/models/MimicMotion.pth", map_location=device)
    # pose_net_state_dict = {k[len('pose_net.'):]: v for k, v in checkpoint.items() if k.startswith('pose_net.')}
    pose_guider = PoseNet(noise_latent_channels=320).to(device=device)
    # pose_guider.load_state_dict(pose_net_state_dict, strict=False)
    ...
    save_dir = "TrainSvd_from_scratch_posevideo_tmp_slim_need_delete"
    if accelerator.is_main_process:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                pose_values = batch["pose_pixels"].to(weight_dtype).to(accelerator.device)
                pixel_values = batch["pixel_values"].to(weight_dtype).to(
                    accelerator.device, non_blocking=True
                )
                #################### modified by yuhang ####################
                random_frame_index = np.random.randint(pixel_values.shape[1])
                # conditional_pixel_values = pixel_values[:, 0:1, :, :, :]
                conditional_pixel_values = pixel_values[:, random_frame_index, :, :, :]

                latents = tensor_to_vae_latent(pixel_values, vae)
                #latents: 1x25x4x64x40
                ...
                encoder_hidden_states = encode_image(
                    pixel_values[:, random_frame_index, :, :, :].float())
                ...
                target = latents
                model_pred = net(inp_noisy_latents, timesteps, encoder_hidden_states, added_time_ids=added_time_ids, pose_values=pose_values)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        unwrap_unet = accelerator.unwrap_model(net.unet)
                        unwrap_pose_guider = accelerator.unwrap_model(net.pose_guider)
                        save_checkpoint(
                            unwrap_unet,
                            save_dir,
                            "denoising_unet",
...
# conda activate /cpfs/data/user/yuhang/anaconda3/env/opensora 
# CUDA_VISIBLE_DEVICES=2 accelerate launch train_from_scratch_posevideo_tmp_slim_need_delete.py --pretrained_model_name_or_path="../../yanghuan/huggingface/stabilityai/stable-video-diffusion-img2vid-xt-1-1"    --per_gpu_batch_size=1 --gradient_accumulation_steps=1     --max_train_steps=400000     --width=576    --height=1024     --checkpointing_steps=2000 --checkpoints_total_limit=1     --learning_rate=1e-5  --lr_warmup_steps=500     --seed=123     --mixed_precision="fp16" --enable_xformers_memory_efficient_attention --validation_steps=0 --gradient_checkpointing