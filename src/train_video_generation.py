...
class DummyDataset(Dataset):
    def __init__(self, num_samples=100000, width=576, height=1024, sample_frames=25):
        ...
        self.total_list = os.listdir('../tiktok/tiktok_videos/')

    def __len__(self):
        return len(self.total_list)

    def __getitem__(self, idx):
        video_path = '../tiktok/tiktok_videos/'+self.total_list[idx]
        skr_path = '../tiktok/tiktok_videos_3pointsface_bodyhand/'+self.total_list[idx]
        vr = decord.VideoReader(video_path)
        skr = decord.VideoReader(skr_path)
        # assert(len(vr) == len(skr))
...
def main():
    ...
    checkpoint = torch.load("../MimicMotion/models/MimicMotion_1-1.pth", map_location=device)
    ...
    save_dir = "TrainSvd_fullhands_slim_3pointsface_1gpu"
    if accelerator.is_main_process:
...
# CUDA_VISIBLE_DEVICES=2 accelerate launch  train_fullhands_slim_3pointsface_1gpu.py --pretrained_model_name_or_path="../../yanghuan/huggingface/stabilityai/stable-video-diffusion-img2vid-xt-1-1"    --per_gpu_batch_size=1 --gradient_accumulation_steps=1     --max_train_steps=200000     --width=576    --height=1024     --checkpointing_steps=5000 --checkpoints_total_limit=1     --learning_rate=1e-5 --lr_warmup_steps=0     --seed=123     --mixed_precision="fp16" --enable_xformers_memory_efficient_attention --validation_steps=0 --gradient_checkpointing